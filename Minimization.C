   ...
   if (status != 0 && status != 1) {
      ...
      if (nrItr > maxRetries) {
         ...
         if (nrItr == 0) { // retry with mu=0 snapshot
            if (w) w->loadSnapshot(mu0Snapshot);
            else cout << "WARNING: workspace not provided, unable to set mu=0 snapshot; will simply retry as is" << endl;
            return minimize(fcn);
         } else if (nrItr == 1) { // retry with nominal snapshot
            if (w) w->loadSnapshot(nominalSnapshot);
            else cout << "WARNING: workspace not provided, unable to set nominal NP snapshot; will simply retry as is" << endl;
            return minimize(fcn);
         }
      }
   }
