                       ...
                       double mu_inj)
{
   ...
   if (doInj) {
      RooDataSet* asimovData_inj = EXOSTATS::makeAsimovData(w, modelConfigName, conditionalExpected, obs_nll, 0, NULL, NULL, -999, true, mu_inj);
   ...
   if (betterBands && doExp) { // no better time than now to do this
      ...
      for (int N = 2; N >= -2; N--) {
         ...
         RooDataSet* asimovData_N = EXOSTATS::makeAsimovData(w, modelConfigName, 1, asimov_0_nll, NtimesSigma, &muStr, &muStrPr, pr_val, 0);
