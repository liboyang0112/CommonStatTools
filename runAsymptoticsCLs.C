...
bool   conditionalExpected = 1 && !doBlind; // Profiling mode for Asimov data: 1 = conditional MLEs, 0 = nominal MLEs
...
bool   doInj               = 0;             // compute expected limit after signal injection
...
bool   doPvals             = true; // perform pvalue calculation
...
void runAsymptoticsCLs(const char *infile, const char *workspaceName, const char *modelConfigName, const char *dataName,
                       const char *asimovDataName, string folder, string mass, double CL, bool betterBands,
                       double mu_inj = 1);
...
void runAsymptoticsCLs(const char *infile, const char *workspaceName = "combWS",
                       ...
                       double CL = 0.95, bool betterBands = false);

double getLimit(RooNLLVar *nll, double initial_guess = 0);
void   getLimit(RooNLLVar *nll, double initial_guess, double &upper_limit, double &muhat);
double getSigma(RooNLLVar *nll, double mu, double muhat, double &qmu);
double getQmu(RooNLLVar *nll, double mu);
void   getExpPvalue(double &pb);
void   getObsPvalue(double mu, double &pv);
...
void runAsymptoticsCLs(const char *infile, const char *workspaceName, const char *modelConfigName, const char *dataName,
                       ...
                       string folder, string mass, double CL, bool doBetterBands)
{
   stringstream smass;
   smass << mass;
...
void runAsymptoticsCLs(const char *infile, const char *workspaceName, const char *modelConfigName, const char *dataName,
                       const char *asimovDataName, string folder, string mass, double CL, bool doBetterBands,
                       double mu_inj)
{
   TStopwatch timer;
   timer.Start();
   ...
   if (doInj) {
      RooDataSet *asimovData_inj =
         EXOSTATS::makeAsimovData(w, modelConfigName, conditionalExpected, obs_nll, 0, NULL, NULL, -999, true, mu_inj);
...
void getLimit(RooNLLVar *nll, double initial_guess, double &upper_limit, double &mu_hat)
{
...
void getObsPvalue(double mu, double &pval)
{
